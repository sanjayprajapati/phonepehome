...
const StudyRoom = ({navigation}) => {
  ...
  const handleButtonPress = useCallback(() => {
    ...
    setTimeout(() => {
    if(btnText == 'on'){
      fetch(HOST+token+'&room=2&allpin=1', {
        method: 'POST',
        headers: {
      ...
      setIsOnn5('ON');
      setIsPinval5('0');
      AsyncStorage.setItem('STR_switchOnestatus', 'ON');
      AsyncStorage.setItem('STR_switchTwostatus', 'ON');
      AsyncStorage.setItem('STR_switchThreestatus', 'ON');
      AsyncStorage.setItem('STR_switchFourstatus', 'ON');
      AsyncStorage.setItem('STR_switchFivestatus', 'ON');
      AsyncStorage.setItem('STR_AllswitchStatus', 'off');
    }else {
      fetch(HOST+token+'&room=2&allpin=0', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      setIsOnn5('OFF');
      setIsPinval5('1');
      AsyncStorage.setItem('STR_switchOnestatus', 'OFF');
      AsyncStorage.setItem('STR_switchTwostatus', 'OFF');
      AsyncStorage.setItem('STR_switchThreestatus', 'OFF');
      AsyncStorage.setItem('STR_switchFourstatus', 'OFF');
      AsyncStorage.setItem('STR_switchFivestatus', 'OFF');
      AsyncStorage.setItem('STR_AllswitchStatus', 'on');
    }
 ...
 useEffect(() => {
  ...
  //getToken();
 // toggleSwitch();
    AsyncStorage.getItem('STR_switchOnestatus').then(value => {
      if (value === 'ON') {        
        setIsOnn1('ON');
        setIsPinval1('0');
    ...
    AsyncStorage.getItem('STR_switchTwostatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('STR_switchThreestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('STR_switchFourstatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('STR_switchFivestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('STR_AllswitchStatus').then(value => {
      if (value === 'off') {        
        setBtnText('off');
      } else {
        setBtnText('on');
  ...
  const switch1OnOff = ()=> {
    ...
    const finalrul = HOST+token+'&room=2&switch1='+isPinval1;
  ...
  const switch2OnOff = ()=> {
   ...
   setTimeout(()=> {
        ...
        if(isOnn2 == 'ON') {
          ...
          const finalrul = HOST+token+'&room=2&switch2='+isPinval2;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=>{
    ...
    if(isOnn3 == 'ON') {
      ...
      AsyncStorage.setItem('STR_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=2&switch3='+isPinval3;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
  ...
  const switch4OnOff = ()=> {
    ...
    if(isOnn4 == 'ON') {
      ...
      AsyncStorage.setItem('STR_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=2&switch4='+isPinval4;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=> {
    ...
    if(isOnn5 == 'ON') {
      ...
      AsyncStorage.setItem('STR_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=2&switch5='+isPinval5;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
    ...
    <View style={styles.container}>
     ...
     <TouchableOpacity 
          ...
          <FontAwesome name="long-arrow-left" size={24} color="#fff"  />
            ...
            <Text style={styles.pageTitle}>Study Room</Text>
            <SpinnerButton
