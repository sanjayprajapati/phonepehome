...
const Kitchen = ({navigation}) => {
  ...
  const handleButtonPress = useCallback(() => {
    ...
    setTimeout(() => {
    if(btnText == 'on'){
      fetch(HOST+token+'&room=1&allpin=1', {
        method: 'POST',
        headers: {
      ...
      setIsOnn5('ON');
      setIsPinval5('0');
      AsyncStorage.setItem('KT_switchOnestatus', 'ON');
      AsyncStorage.setItem('KT_switchTwostatus', 'ON');
      AsyncStorage.setItem('KT_switchThreestatus', 'ON');
      AsyncStorage.setItem('KT_switchFourstatus', 'ON');
      AsyncStorage.setItem('KT_switchFivestatus', 'ON');
      AsyncStorage.setItem('KT_AllswitchStatus', 'off');
    }else {
      fetch(HOST+token+'&room=1&allpin=0', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      setIsOnn5('OFF');
      setIsPinval5('1');
      AsyncStorage.setItem('KT_switchOnestatus', 'OFF');
      AsyncStorage.setItem('KT_switchTwostatus', 'OFF');
      AsyncStorage.setItem('KT_switchThreestatus', 'OFF');
      AsyncStorage.setItem('KT_switchFourstatus', 'OFF');
      AsyncStorage.setItem('KT_switchFivestatus', 'OFF');
      AsyncStorage.setItem('KT_AllswitchStatus', 'on');
    }
 ...
 useEffect(() => {
  ...
  //getToken();
 // toggleSwitch();
    AsyncStorage.getItem('KT_switchOnestatus').then(value => {
      if (value === 'ON') {        
        setIsOnn1('ON');
        setIsPinval1('0');
    ...
    AsyncStorage.getItem('KT_switchTwostatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('KT_switchThreestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('KT_switchFourstatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('KT_switchFivestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('KT_AllswitchStatus').then(value => {
      if (value === 'off') {        
        setBtnText('off');
      } else {
        setBtnText('on');
  ...
  const switch1OnOff = ()=> {
    ...
    const finalrul = HOST+token+'&room=1&switch1='+isPinval1;
  ...
  const switch2OnOff = ()=> {
   ...
   setTimeout(()=> {
        ...
        if(isOnn2 == 'ON') {
          ...
          const finalrul = HOST+token+'&room=1&switch2='+isPinval2;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=>{
    ...
    if(isOnn3 == 'ON') {
      ...
      AsyncStorage.setItem('KT_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=1&switch3='+isPinval3;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
  ...
  const switch4OnOff = ()=> {
    ...
    if(isOnn4 == 'ON') {
      ...
      AsyncStorage.setItem('KT_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=1&switch4='+isPinval4;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=> {
    ...
    if(isOnn5 == 'ON') {
      ...
      AsyncStorage.setItem('KT_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=1&switch5='+isPinval5;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
    ...
    <View style={styles.container}>
     ...
     <TouchableOpacity 
          ...
          <FontAwesome name="long-arrow-left" size={24} color="#fff"  />
            ...
            <Text style={styles.pageTitle}>Kitchen</Text>
            <SpinnerButton
