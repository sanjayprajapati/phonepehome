...
const BedroomTwo = ({navigation}) => {
  ...
  const handleButtonPress = useCallback(() => {
    ...
    setTimeout(() => {
    if(btnText == 'on'){
      fetch(HOST+token+'&room=5&allpin=1', {
        method: 'POST',
        headers: {
      ...
      setIsOnn5('ON');
      setIsPinval5('0');
      AsyncStorage.setItem('BRT_switchOnestatus', 'ON');
      AsyncStorage.setItem('BRT_switchTwostatus', 'ON');
      AsyncStorage.setItem('BRT_switchThreestatus', 'ON');
      AsyncStorage.setItem('BRT_switchFourstatus', 'ON');
      AsyncStorage.setItem('BRT_switchFivestatus', 'ON');
      AsyncStorage.setItem('BRT_AllswitchStatus', 'off');
    }else {
      fetch(HOST+token+'&room=5&allpin=0', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      setIsOnn5('OFF');
      setIsPinval5('1');
      AsyncStorage.setItem('BRT_switchOnestatus', 'OFF');
      AsyncStorage.setItem('BRT_switchTwostatus', 'OFF');
      AsyncStorage.setItem('BRT_switchThreestatus', 'OFF');
      AsyncStorage.setItem('BRT_switchFourstatus', 'OFF');
      AsyncStorage.setItem('BRT_switchFivestatus', 'OFF');
      AsyncStorage.setItem('BRT_AllswitchStatus', 'on');
    }
 ...
 useEffect(() => {
  ...
  //getToken();
 // toggleSwitch();
    AsyncStorage.getItem('BRT_switchOnestatus').then(value => {
      if (value === 'ON') {        
        setIsOnn1('ON');
        setIsPinval1('0');
    ...
    AsyncStorage.getItem('BRT_switchTwostatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('BRT_switchThreestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('BRT_switchFourstatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('BRT_switchFivestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('BRT_AllswitchStatus').then(value => {
      if (value === 'off') {        
        setBtnText('off');
      } else {
        setBtnText('on');
  ...
  const switch1OnOff = ()=> {
    ...
    const finalrul = HOST+token+'&room=5&switch1='+isPinval1;
  ...
  const switch2OnOff = ()=> {
   ...
   setTimeout(()=> {
        ...
        if(isOnn2 == 'ON') {
          ...
          const finalrul = HOST+token+'&room=5&switch2='+isPinval2;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=>{
    ...
    if(isOnn3 == 'ON') {
      ...
      AsyncStorage.setItem('BRT_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=5&switch3='+isPinval3;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
  ...
  const switch4OnOff = ()=> {
    ...
    if(isOnn4 == 'ON') {
      ...
      AsyncStorage.setItem('BRT_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=5&switch4='+isPinval4;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
  ...
  const switch5OnOff = ()=> {
   ...
   setTimeout(()=> {
    if(isOnn5 == 'OFF') {
      setIsPinval5('0');
      setIsOnn5('ON');
      AsyncStorage.setItem('LR_switchFivestatus', 'ON');
      setBtnText('off');
      AsyncStorage.setItem('LR_AllswitchStatus', 'off');
    }
    if(isOnn5 == 'ON') {
      setIsPinval5('1');
      setIsOnn5('OFF');
      AsyncStorage.setItem('BT_switchFivestatus', 'OFF');
      setBtnText('on');
      AsyncStorage.setItem('BT_AllswitchStatus', 'on');
    }
    
          const finalrul = HOST+token+'&room=5&switch5='+isPinval5;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
    ...
    <View style={styles.container}>
     ...
     <TouchableOpacity 
          ...
          <FontAwesome name="long-arrow-left" size={24} color="#fff"  />
            ...
            <Text style={styles.pageTitle}>Bed Room Two</Text>
            <SpinnerButton
