...
import {View, Button,ImageBackground , Switch, StyleSheet, Text, TextInput, ScrollView,TouchableOpacity,ActivityIndicator} from 'react-native';
...
const LivingRoom = ({navigation}) => {
  ...
  const handleButtonPress = useCallback(() => {
    ...
    setTimeout(() => {
    if(btnText == 'on'){
      fetch(HOST+token+'&room=0&allpin=1', {
        method: 'POST',
        headers: {
      ...
      setIsOnn5('ON');
      setIsPinval5('0');
      AsyncStorage.setItem('LR_switchOnestatus', 'ON');
      AsyncStorage.setItem('LR_switchTwostatus', 'ON');
      AsyncStorage.setItem('LR_switchThreestatus', 'ON');
      AsyncStorage.setItem('LR_switchFourstatus', 'ON');
      AsyncStorage.setItem('LR_switchFivestatus', 'ON');
      AsyncStorage.setItem('LR_AllswitchStatus', 'off');
    }else {
      fetch(HOST+token+'&room=0&allpin=0', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      setIsOnn5('OFF');
      setIsPinval5('1');
      AsyncStorage.setItem('LR_switchOnestatus', 'OFF');
      AsyncStorage.setItem('LR_switchTwostatus', 'OFF');
      AsyncStorage.setItem('LR_switchThreestatus', 'OFF');
      AsyncStorage.setItem('LR_switchFourstatus', 'OFF');
      AsyncStorage.setItem('LR_switchFivestatus', 'OFF');
      AsyncStorage.setItem('LR_AllswitchStatus', 'on');
    }
  ...
  //         setEmail(data.email);
  //         setAppart(data.appartment_type);
  //         console.log(appart)
  //         //console.log(email)
  //       }
  //     });
 ...
 useEffect(() => {
  ...
  //getToken();
 // toggleSwitch();
    AsyncStorage.getItem('LR_switchOnestatus').then(value => {
      if (value === 'ON') {        
        setIsOnn1('ON');
        setIsPinval1('0');
    ...
    AsyncStorage.getItem('LR_switchTwostatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('LR_switchThreestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('LR_switchFourstatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('LR_switchFivestatus').then(value => {
      if (value === 'ON') {
    ...
    AsyncStorage.getItem('LR_AllswitchStatus').then(value => {
      if (value === 'off') {        
        setBtnText('off');
      } else {
        setBtnText('on');
  ...
  const switch1OnOff = ()=> {
    ...
    const finalrul = HOST+token+'&room=0&switch1='+isPinval1;
  ...
  const switch2OnOff = ()=> {
   ...
   setTimeout(()=> {
        ...
        if(isOnn2 == 'ON') {
          ...
          const finalrul = HOST+token+'&room=0&switch2='+isPinval2;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=>{
    ...
    if(isOnn3 == 'ON') {
      ...
      AsyncStorage.setItem('LR_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=0&switch3='+isPinval3;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
  ...
  const switch4OnOff = ()=> {
    ...
    if(isOnn4 == 'ON') {
      ...
      AsyncStorage.setItem('LR_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=0&switch4='+isPinval4;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
   ...
   setTimeout(()=> {
    ...
    if(isOnn5 == 'ON') {
      ...
      AsyncStorage.setItem('LR_AllswitchStatus', 'on');
          ...
          const finalrul = HOST+token+'&room=0&switch5='+isPinval5;
          console.log(finalrul)
          fetch(finalrul, {
            method: 'POST',
            headers: {
    ...
    <View style={styles.container}>
     <View style={styles.header}>
          ...
          <FontAwesome name="long-arrow-left" size={24} color="#fff"  />
            ...
            <Text style={styles.pageTitle}>Living Room</Text>
            <SpinnerButton
